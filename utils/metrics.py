...
def aver_psnr_ds(img1, img2, to_int = True,cut=30):
    ...
    for i in range(im_len):
        ...
        PSNR += psnr(img1[i], img2[i],cut)
    return PSNR / len(img1)


def aver_psnr_ds_microscopy(img1, img2, to_int=True):
    ''' For images with different size and stored by a list'''
    im_len = len(img1)
    assert len(img1) == len(img2)
    for i in range(im_len):
...
def aver_ssim_ds(img1, img2, to_int=True,cut=30):
    ''' For images with different size and stored by a list'''
    im_len = len(img1)
    assert len(img1) == len(img2)
    for i in range(im_len):
    ...
    for i in range(im_len):
        ...
        SSIM += ssim(img1[i], img2[i],cut)
    return SSIM / len(img1)


def aver_ssim_ds_microscopy(img1, img2, to_int=True):
    ''' For images with different size and stored by a list'''
    im_len = len(img1)
    assert len(img1) == len(img2)
    for i in range(im_len):
